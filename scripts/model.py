...
class CS2BotModel(nn.Module):
    def __init__(self, state_dim=22, hidden=256):
        ...
        self.fusion = nn.Sequential(
            nn.Linear(128 + hidden, 256), nn.LayerNorm(256),
            nn.GELU(), nn.Linear(256, 128), nn.GELU(),
        )
        self.move_head   = nn.Sequential(nn.Linear(128, 64), nn.GELU(), nn.Linear(64, 5), nn.Sigmoid())
        self.aim_head    = nn.Sequential(nn.Linear(128, 64), nn.GELU(), nn.Linear(64, 2), nn.Tanh())
        self.action_head = nn.Sequential(nn.Linear(128, 64), nn.GELU(), nn.Linear(64, 4), nn.Sigmoid())

    def forward(self, state_seq, lstm_hidden=None):
        B, T, D = state_seq.shape
        ticks = self.tick_encoder(state_seq.reshape(B * T, D)).reshape(B, T, 128)
        tcn_last = self.tcn(ticks.transpose(1, 2))[:, :, -1]
        lstm_out, lstm_hidden = self.lstm(ticks, lstm_hidden)
        fused = self.fusion(torch.cat([tcn_last, lstm_out[:, -1, :]], dim=-1))
        return {"move": self.move_head(fused), "aim": self.aim_head(fused), "action": self.action_head(fused)}, lstm_hidden
